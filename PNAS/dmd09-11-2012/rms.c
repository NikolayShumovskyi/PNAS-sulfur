...
int nonnative(int p, int q, int ct)
{
  int  nnn=0;
  well_type k=ct;
...
extern int init_rms(void)
{
  ...
  if(!cont)return 0;
  cont[0]=(int *)malloc(ncont*sizeof(int));
  if(!cont[0])return 0;
  for(i=0;i<nn-1;i++)
    cont[i+1]=cont[i]+i;
  for(i=0;i<ncont;i++)
  ...
  nuc1=(int *)malloc(nnuc*sizeof(int));
  if(!nuc1){fclose(infile);nnuc=0;return good;}
  nuc2=(int *)malloc(nnuc*sizeof(int));
...
int contact(int i, int j)
{
  well_type ct=bond((int)i,(int)j);
  if(ct<0)return 0;
